...
function filterByName(first_name){
  for(i=0;i<persons.length;i++){
    if (persons[i].first_name === first_name){
      return persons[i];
    }
  }
  return "not found";
...
//Use Array.map to return an array of all the email fields.
let emails = persons.map(({email})=> email);
console.log(emails);

// Use Array.sort to return the array sorted in descending order by date_of_birth.
let sortedPersons = persons.sort((a,b)=> new Date(b.date_of_birth) - new Date(a.date_of_birth));
